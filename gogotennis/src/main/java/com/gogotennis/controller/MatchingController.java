...
public class MatchingController {
	...
	public String matchingDetail(@PathVariable("matchingId") Long matchingId, Model model,
		...
		List<Player> playersA = players.stream().filter(m -> m.getTeam().toString().equals("A")).collect(Collectors.toList());
		List<Player> playersB = players.stream().filter(m -> m.getTeam().toString().equals("B")).collect(Collectors.toList());
